...
export function AdminMessages() {
  ...
  const handleReply = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!activeThread) return;
    const content = (e.currentTarget as HTMLFormElement).content.value as string;
    if (!content.trim()) return;

    try {
      await axios.post(`/api/admin/threads/${activeThread}/reply`, {
        message: content.trim(),
        senderName: "Admin Support"
      });
      (e.target as HTMLFormElement).reset();
      toast.success("Reply recorded and synced");
    } catch (err) {
          ...
          <>
            ...
            <form onSubmit={handleReply} className="p-4 border-t border-gray-100 bg-white flex gap-2">
              <input 
                ...
                required
              />
              <button className="bg-primary text-black px-6 rounded-xl text-xs font-bold uppercase tracking-widest hover:bg-primary transition-colors">
                Send
              </button>
            </form>
          </>
