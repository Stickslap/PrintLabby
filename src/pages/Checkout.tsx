...
export function Checkout() {
  const { cart, rewardBalance } = useStore();
  ...
  useEffect(() => {
    const initBC = async () => {
      ...
      try {
        ...
        // Fetch full checkout details to sync initial totals/coupons
        try {
          const checkRes = await axios.get(`/api/checkout/${data.checkoutId}`);
          if (checkRes.data?.data) {
            setCheckoutTotals(checkRes.data.data);
            setAppliedCoupons(checkRes.data.data.coupons || []);
          }
        } catch (e) {
          console.error("Initial checkout fetch failed", e);
        }
  ...
  const grandTotal = (checkoutTotals?.grand_total || (subtotal + shippingCost));
  const finalTotal = Math.max(0, grandTotal - (useRewards ? appliedCredit : 0));
    ...
    try {
      ...
      if (response.data?.success) {
        ...
        localStorage.setItem('recentOrderSummary', JSON.stringify({
          ...
          deliveryTimeframe: selectedShipping?.description || '3-7 Business Days',
          total: finalTotal
        }));
  ...
  const handleApplyCoupon = async () => {
    if (!couponCode || !checkoutId) return;

    setIsApplyingCoupon(true);
    try {
      await axios.post(`/api/checkout/${checkoutId}/coupons`, { couponCode });
      const { data } = await axios.get(`/api/checkout/${checkoutId}`);
      
      if (data.data) {
        setCheckoutTotals(data.data);
        setAppliedCoupons(data.data.coupons || []);
  ...
  const handleRemoveCoupon = async (code: string) => {
    ...
    try {
      await axios.delete(`/api/checkout/${checkoutId}/coupons/${code}`);
      const { data } = await axios.get(`/api/checkout/${checkoutId}`);
  ...
  return (
    <div className="min-h-screen flex flex-col font-sans bg-white text-black">
      {/* Header */}
      <header className="px-8 py-6 border-b border-gray-200 flex justify-center items-center">
            ...
            <form id="checkout-form" onSubmit={preventDefaultSubmit} className="space-y-12">
              ...
              <section>
                <div className="flex justify-between items-center mb-4">
                  ...
                  {!user && (
                    <span className="text-[10px] font-bold uppercase tracking-widest text-gray-500">
                      Have an account? <Link to="/login" className="text-amber-500 hover:text-amber-600">Sign in</Link>
                    </span>
                  )}
                </div>
                <div className="space-y-4">
                  <input 
                    ...
                    required
                    className="w-full bg-blue-50/50 border border-blue-100 rounded-md px-4 py-3 text-sm focus:outline-none focus:ring-1 focus:ring-amber-400"
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <input 
                      ...
                      className="w-full border border-gray-200 rounded-md px-4 py-3 text-sm focus:outline-none focus:border-amber-400" 
                    />
                    <input 
                      type="text" 
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    ...
                    <select 
                      ...
                      required
                      className="w-full border border-gray-200 rounded-md px-4 py-3 text-sm focus:outline-none focus:border-amber-400 text-gray-500 uppercase"
                    >
              ...
              <section>
                <div className="flex items-center justify-between mb-4">
                  ...
                  {isFetchingShipping && <span className="text-[8px] font-black uppercase text-amber-500 animate-pulse">Syncing store options...</span>}
                </div>
                  ...
                  {shippingOptions.length > 0 ? shippingOptions.map((option) => (
                    <label 
                      ...
                      className={`flex items-center justify-between p-4 border rounded-md cursor-pointer transition-all ${
                        ...
                        ? 'border-amber-400 bg-purple-50/30' 
              ...
              <section className="bg-gray-50 border border-gray-100 rounded-lg p-6">
                ...
                <div className="mb-5">
                  ...
                  <input
                    ...
                    className="w-full bg-white border border-gray-200 rounded-md px-4 py-3.5 text-[10px] font-black tracking-widest uppercase focus:outline-none focus:border-amber-400 focus:ring-2 focus:ring-amber-100 transition-all placeholder:text-gray-300"
                  />
                </div>
    ...
    locId: import.meta.env.VITE_SQUARE_LOCATION_ID || 'missing'
              ...
              <div className="space-y-3">
                <label className="flex items-center p-4 border border-gray-200 rounded-md cursor-pointer hover:bg-gray-50">
                  <input type="checkbox" required className="w-4 h-4 text-amber-600 border-gray-300 rounded focus:ring-amber-500" />
                  <span className="ml-3 text-[10px] font-bold uppercase tracking-widest text-black">
                    I accept Print Labby LLC <a href="#" className="text-amber-500 underline decoration-amber-200 underline-offset-2">Terms of Service and Privacy Terms</a>.
                  </span>
                </label>
                <label className="flex items-center p-4 border border-gray-200 rounded-md cursor-pointer hover:bg-gray-50">
                  <input type="checkbox" required className="w-4 h-4 text-amber-600 border-gray-300 rounded focus:ring-amber-500" />
                  <span className="ml-3 text-[10px] font-bold uppercase tracking-widest text-black">
                    Print Labby manufacturing standard with a transparent background.
                  </span>
                </label>
              </div>
        ...
        <div className="w-full lg:w-[40%] bg-[#f9fafb] border-t lg:border-t-0 p-6 lg:p-12 lg:pr-32">
              ...
              return (
                <div key={idx} className="flex gap-4 items-start">
                  <div className="relative">
                    <div className="w-16 h-16 bg-gray-200 border border-gray-200 rounded-lg overflow-hidden relative shadow-sm flex-shrink-0">
                      {item.primary_image ? (
                    ...
                    {item.artworkDataUrl && (
                      <div className="absolute -bottom-1 -left-1 w-8 h-8 bg-white border border-gray-200 rounded-md overflow-hidden shadow-lg z-20">
                        <img src={item.artworkDataUrl} alt="Artwork" className="w-full h-full object-cover" />
                      </div>
                    )}
                    <div className="absolute -top-2 -right-2 bg-amber-500 text-white w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold z-10 shadow-sm border border-white">
                      {item.quantity}
                    </div>
                  </div>
                  <div className="flex-1 pt-1">
                    ...
                    {item.artworkNotes && (
                      <p className="text-[9px] text-amber-600 uppercase font-bold tracking-widest mt-1 leading-relaxed italic">
                        Note: “{item.artworkNotes}”
                      </p>
                    )}
                  </div>
                </div>
          ...
          {user && rewardBalance > 0 ? (
            <div className="bg-amber-50/50 border border-amber-100 rounded-xl p-6 mb-8">
              <div className="flex items-center justify-between mb-4">
                 <div className="flex items-center gap-3">
                    <div className="w-8 h-8 bg-[#FFD700] rounded-full flex items-center justify-center text-[10px] text-orange-900 font-black shadow-sm">C</div>
                    <div>
                      <h4 className="text-[11px] font-black uppercase tracking-widest text-black">Reward Registry</h4>
                      <p className="text-[9px] font-bold text-gray-400 uppercase tracking-widest">Available: ${rewardBalance.toFixed(2)}</p>
                    </div>
                 </div>
                 <button 
                  type="button"
                  onClick={() => {
                    if (useRewards) {
                      ...
                      setUseRewards(true);
                      setAppliedCredit(Math.min(rewardBalance, grandTotal));
                    }
                  }}
                  className={`px-4 py-2 rounded-full text-[9px] font-black uppercase tracking-widest transition-all ${
                    useRewards ? 'bg-black text-white' : 'bg-white border border-gray-200 text-gray-500 hover:border-amber-400'
                  }`}
                 >
                   {useRewards ? 'Remove' : 'Apply Balance'}
                 </button>
              </div>
              
              {useRewards && (
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="pt-4 border-t border-amber-100 flex items-center justify-between">
                   <p className="text-[10px] font-black uppercase tracking-widest text-orange-700 italic">Credit applied instantly</p>
                   <p className="text-sm font-black text-orange-600">-${appliedCredit.toFixed(2)}</p>
                </motion.div>
              )}
            </div>
          ) : !user && (
            <div className="bg-gray-50 border border-gray-100 rounded-xl p-6 mb-8">
              <p className="text-[10px] font-bold text-gray-400 uppercase tracking-[0.2em] text-center">
                <Link to="/login" className="text-primary hover:underline">Sign in</Link> to earn 3% store credit on this project.
                <br/><span className="text-[8px] opacity-70">(Guest accounts do not earn rewards)</span>
              </p>
            </div>
          )}
          ...
          <div className="space-y-4 mb-8">
            <div className="flex gap-2">
              <input 
                ...
                className="flex-1 border border-gray-300 bg-white rounded text-[10px] uppercase font-bold tracking-widest px-4 py-3 outline-none focus:border-amber-400 placeholder:text-gray-400"
              />
              <button 
                onClick={handleApplyCoupon}
                disabled={isApplyingCoupon || !couponCode}
                className="bg-white border text-gray-500 border-gray-300 rounded px-6 py-3 text-[10px] font-black uppercase tracking-widest hover:bg-gray-50 transition-colors disabled:opacity-50"
              >
            ...
            {appliedCoupons.length > 0 && (
              <div className="flex flex-wrap gap-2">
                {appliedCoupons.map((coupon, idx) => (
                  <div key={idx} className="flex items-center gap-2 bg-amber-50 border border-amber-100 rounded-full px-3 py-1">
                    <span className="text-[10px] font-black uppercase tracking-tighter text-amber-700 italic">
                      {coupon.code}
                    </span>
                    <button 
                      onClick={() => handleRemoveCoupon(coupon.code)}
                      className="text-amber-400 hover:text-amber-600 font-bold text-xs"
                    >
                      ×
                    </button>
                  </div>
          ...
          <div className="space-y-4 mb-6">
            <div className="flex justify-between items-center text-[11px] font-black uppercase tracking-widest text-gray-500">
              <span>Subtotal</span>
              <span className="text-black">${cart.length ? (checkoutTotals?.subtotal_inc_tax || subtotal).toFixed(2) : '10.00'}</span>
            </div>
            {checkoutTotals?.cart_level_discount_total > 0 && (
              <div className="flex justify-between items-center text-[11px] font-black uppercase tracking-widest text-green-600">
                <span>Discount</span>
                <span>-${checkoutTotals.cart_level_discount_total.toFixed(2)}</span>
              </div>
            )}
            <div className="flex justify-between items-center text-[11px] font-black uppercase tracking-widest text-gray-500">
              <span>Shipping</span>
            ...
            {useRewards && appliedCredit > 0 && (
              <div className="flex justify-between items-center text-[11px] font-black uppercase tracking-widest text-orange-600">
