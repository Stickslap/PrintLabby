...
export function Admin() {
    ...
    const unsub = onAuthStateChanged(auth, (user) => {
      ...
      if (user) {
        ...
        if (ADMIN_EMAILS.includes(email) || email.toLowerCase().includes("arlington.a.teheran")) {
          ...
          setAdminUser({ displayName: "Staff Terminal", email: "staff@printlabby.com", isAnonymous: true, uid: user.uid });
        } else if (!isStaff) {
          logout();
        ...
        setAdminUser({ displayName: "Staff Terminal", email: "staff@printlabby.com", pendingAuth: true });
      } else {
        navigate("/admin/login");
      }
    });
  ...
  const fetchData = async () => {
    try {
      const [statsRes, ordersRes, customersRes, statusRes] = await Promise.all([
  ...
  const handleStatusChange = async (orderId: string | number, statusId: string) => {
    try {
      await axios.put(`/api/admin/orders/${orderId}/status`, { status_id: parseInt(statusId) });
      toast.success(`Order #${orderId} updated`);
      // Update local state to reflect change immediately
      setOrders(prev => prev.map(o => o.id === orderId ? { ...o, status_id: parseInt(statusId), status: orderStatuses.find(s => String(s.id) === statusId)?.status || o.status } : o));
    } catch (err) {
      ...
      <aside className={`fixed left-0 top-0 bottom-0 w-64 bg-white border-r border-gray-200 pt-24 px-0 flex flex-col z-50 transition-transform duration-300 lg:translate-x-0 overflow-y-auto ${
        ...
        <div className="mx-4 mb-8 mt-auto p-4 bg-secondary rounded-2xl">
          ...
          <a 
            ...
            target="_blank" 
            className="flex items-center justify-between bg-primary text-black px-3 py-2 rounded-lg text-[10px] font-black uppercase"
          >
          ...
          <header className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Print Labby Operational Hub • v1.0.4</p>
              </div>
            </div>
                          ...
                          >
                            ...
                            <td className="py-4 px-6">
                              <select 
                                ...
                                className={`text-[9px] font-black px-2 py-1 rounded uppercase bg-white border border-gray-200 outline-none focus:border-primary transition-all cursor-pointer ${getOrderStatusStyle(order.status)}`}
                              >
                                {orderStatuses.map(s => (
                                  <option key={s.id} value={s.id} className="bg-white text-gray-900 italic">
                                    {s.status === 'Awaiting Shipment' ? 'Packing' : s.status}
                                  </option>
                                ))}
                              </select>
                            </td>
                            ...
                            <td className="py-4 px-6 text-right">
                              <button 
                                ...
                                className="p-1 px-3 bg-primary text-black hover:brightness-90 rounded transition-all text-[10px] font-bold uppercase flex items-center gap-2 ml-auto"
                              >
                          ...
                          <td className="py-4 px-6 text-right">
                            <div className="flex justify-end gap-2">
                              ...
                              <button 
                                ...
                                className="p-2 px-4 bg-primary text-black hover:brightness-90 rounded-lg transition-all text-[10px] font-bold uppercase flex items-center gap-2"
                              >
