  ...
  if (cart.length === 0) {
    return (
      <div className="pt-60 pb-40 text-center px-6">
        <ShoppingCart className="w-20 h-20 mx-auto mb-8 text-amber-700" />
        ...
        <Link 
          to="/shop" 
          className="inline-flex items-center justify-center bg-primary text-black px-12 py-5 rounded-full text-sm font-bold uppercase tracking-wider hover:opacity-90 transition-all shadow-lg shadow-primary/20"
        >
          Browse the drops
        </Link>
      </div>
    ...
    <div className="pt-32 pb-20 bg-[#fafafa] min-h-screen">
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-12 gap-6">
          <Link to="/shop" className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest text-gray-500 hover:text-amber-700 transition-colors">
            <ArrowLeft className="w-3 h-3" /> Continue Shopping
          </Link>
          <button 
            onClick={() => useStore.getState().clearCart()}
            className="text-[10px] font-bold border-b border-gray-400 uppercase tracking-widest hover:text-amber-700 hover:border-[#5719D3] transition-colors pb-0.5 text-gray-500"
          >
            Clear Cart
          </button>
        </div>
                    ...
                    <div className="flex-1 flex flex-col">
                      <div className="flex justify-between items-start mb-2 gap-4">
                        <div>
                          ...
                          <h3 className="text-xl sm:text-2xl font-headline font-black uppercase tracking-tight italic group-hover:text-amber-700 transition-colors">
                            <Link to={`/product/${item.id}`}>{item.name}</Link>
                          </h3>
                        </div>
                        <p className="text-2xl font-headline font-black italic tracking-tighter text-amber-700">${(Number(item.price || 0) * item.quantity).toFixed(2)}</p>
                      </div>
          ...
          <div className="lg:col-span-4">
            <div className="bg-white border border-gray-200 rounded-3xl p-8 shadow-sm lg:sticky lg:top-32">
              ...
              <div className="bg-gray-50 rounded-2xl p-6 mb-8 flex justify-between items-center border border-gray-100">
                ...
                <span className="text-4xl font-headline font-black italic tracking-tighter text-amber-700">${(subtotal || 0).toFixed(2)}</span>
              </div>

              <Link 
                to="/checkout"
                className="w-full bg-primary text-black py-5 rounded-xl text-sm font-black uppercase tracking-widest hover:brightness-90 transition-all flex items-center justify-center gap-3 shadow-lg shadow-amber-500/20 group"
              >
